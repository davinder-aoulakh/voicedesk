...
import { Calendar, Plus, Search, Check, X, Clock, User } from 'lucide-react';
...
export default function Bookings() {
  ...
  const load = async () => {
    ...
    const data = await base44.entities.Booking.filter({ business_id: businesses[0].id }, '-created_date', 100);
    setBookings(data);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    if (search) data = data.filter(b => (b.customer_name || '').toLowerCase().includes(search.toLowerCase()) || (b.customer_phone || '').includes(search));
    setFiltered(data);
